...
class Pos {
  ...
  static const MethodChannel _channel = MethodChannel('pos');
  ...
  static Future<void> printRow({required List<ColumnMaker> cols}) async {
    final _jsonCols = List<Map<String, String>>.from(
      cols.map<Map<String, String>>(
        (ColumnMaker col) => col.toJson(),
      ),
    );
    // ((384 / fontsize) << 1)-(number of columns + 1)
  ...
  static Future<void> printImage(
    Uint8List img, {
    SunmiPrintAlign alignment = SunmiPrintAlign.CENTER,
    int size = 8,
  }) async {
    Map<String, dynamic> arguments = <String, dynamic>{
  ...
  static Future<void> performPrint() async {
    await _channel.invokeMethod("PERFORM_PRINT");
  }
